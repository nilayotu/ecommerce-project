...
export default function Slider() {
  ...
  return (
    <div className="relative w-full h-[450px] sm:h-[500px] md:h-[600px] lg:h-[1000px] overflow-hidden">
      {items.map((item) => (
        ...
        >
          ...
          <div className="relative z-10 text-center text-[#FFFFFF] px-4">
            <h2 className="text-3xl sm:text-3xl md:text-8xl md:font-bold font-semibold mb-2 sm:mb-4">
              {item.title}
            </h2>
            ...
            <button className="bg-[#23A6F0] text-[#FFFFFF] md:px-20 md:text-4xl md:py-6 px-10 py-3 sm:px-6 rounded font-bold text-md sm:text-base">
              {item.btnText}
            </button>
          </div>
        </div>
      ...
      <button className="absolute left-2 sm:left-2 top-1/2 -translate-y-1/2 text-[#FFFFFF] z-20">
        <ChevronLeft size={60} strokeWidth={1} />
        </button>
        <button className="absolute right-2 sm:right-2 top-1/2 -translate-y-1/2 text-[#FFFFFF] z-20">
